...
class RawMaterialStockCountController extends Controller
{
    public function index()
    {
        ...
        if( $role->hasPermissionTo('rawmaterials-index') ) {
            ...
            return view('backend.rawmaterial.stock_count.index', compact('lims_warehouse_list', 'lims_brand_list', 'lims_category_list', 'lims_stock_count_all', 'general_setting'));
        }
        else
    ...
    {
        $data = $request->all();
        $data['material_type'] = 'raw_material'; // Set material type
        ...
        // Get raw materials (no warehouse pivot table, so we get all active raw materials)
        if( isset($data['brand_id']) && isset($data['category_id']) ){
            $lims_raw_material_list = RawMaterial::whereIn('category_id', $data['category_id'])
        ...
        elseif( isset($data['brand_id']) ){
            $lims_raw_material_list = RawMaterial::whereIn('brand_id', $data['brand_id'])
                ->where('is_active', true)
                ->select('name', 'code', 'qty')
                ->get();
        ...
        if( count($lims_raw_material_list) ){
            $csvData=array('Raw Material Name, Raw Material Code, IMEI or Serial Numbers, Counted');
            foreach ($lims_raw_material_list as $raw_material) {
                $csvData[]=$raw_material->name.','.$raw_material->code.',,'.($raw_material->qty ?? 0);
            }
            if (!file_exists(public_path().'/stock_count/raw_material/')) {
                mkdir(public_path().'/stock_count/raw_material/', 0777, true);
            }
            $filename= date('Ymd').'-'.date('his'). ".csv";
            $file_path= public_path().'/stock_count/raw_material/'.$filename;
            ...
            $data['reference_no'] = 'rm-scr-' . date("Ymd") . '-'. date("his");
    ...
    public function finalize(Request $request)
    {
        ...
        $document->move(public_path('stock_count/raw_material/'), $documentName);
    ...
    public function stockDif($id)
    {
        $lims_stock_count_data = StockCount::where('id', $id)
            ->where('material_type', 'raw_material')
            ->firstOrFail();
        $file_handle = fopen(public_path('stock_count/raw_material/').$lims_stock_count_data->final_file, 'r');
        $i = 0;
        $temp_dif = -1000000;
        $data = [];
        $raw_material = [];
        ...
        while( !feof($file_handle) ) {
            ...
            if( $current_line && $i > 0){
                $raw_material_data = RawMaterial::select('id','code','cost','qty')
                ...
                if($raw_material_data) {
                    $raw_material[] = $current_line[0].' ['.$raw_material_data->code.']';
                    $expected[] = $raw_material_data->qty;
                    if(isset($current_line[3]) && $current_line[3]){
                        $difference[] = $temp_dif = $current_line[3] - $raw_material_data->qty;
                        $counted[] = $current_line[3];
                    }
                    else{
                        $difference[] = $temp_dif = $raw_material_data->qty * (-1);
                        $counted[] = 0;
                    }
                    $cost[] = ($raw_material_data->cost ?? 0) * $temp_dif;
                }
            }
            $i++;
        }
    ...
    public function qtyAdjustment($id)
    {
        ...
        while( !feof($file_handle) ) {
            $current_line = fgetcsv($file_handle);
            if( $current_line && $i > 0 ){
                $raw_material_data = RawMaterial::select('id','code','qty')->where('code', $current_line[1])->first();
                if($raw_material_data) {
                    ...
                    if(isset($current_line[3]) && $current_line[3])
                        $temp_qty = $current_line[3] - $raw_material_data->qty;
                    else
                        $temp_qty = $raw_material_data->qty * (-1);
        ...
        fclose($file_handle);
        if(isset($raw_material_id) && count($raw_material_id) > 0) {
            return view('backend.rawmaterial.stock_count.qty_adjustment', compact('lims_warehouse_list', 'warehouse_id', 'id', 'raw_material_id', 'names', 'code', 'qty', 'action'));
        } else {
