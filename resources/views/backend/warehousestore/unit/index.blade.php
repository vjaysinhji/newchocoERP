                    ...
                    <td>
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                                ...
                                <li class="divider"></li>
                                {{ Form::open(['route' => ['warehouse-stores.unit.destroy', $unit->id], 'method' => 'DELETE'] ) }}
                                <li>
...
<div id="createUnitModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
  <div role="document" class="modal-dialog">
      <div class="modal-content">
          {!! Form::open(['route' => 'warehouse-stores.unit.store', 'method' => 'post']) !!}
...
<script type="text/javascript">
    ...
    $("ul#basement #warehouse-store-unit-menu").addClass("active");
    ...
    $(document).on('click', '.open-EditUnitDialog', function(e) {
        ...
        var url = "{{ url('warehouse-stores/unit') }}/" + id + "/edit";
        ...
        $.get(url, function(data) {
            ...
            var updateUrl = "{{ url('warehouse-stores/unit') }}/" + (data.id || data['id']);
    ...
    $(document).on('click', '#create_unit', function(e) {
        ...
        $.ajax({
            url: form.attr('action') || '{{ route("warehouse-stores.unit.store") }}',
            method: 'POST',
            data: formData,
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                ...
                action: function ( e, dt, node, config ) {
                    if(user_verified == '1') {
                        ...
                        if(unit_id.length && confirm("Are you sure want to delete?")) {
                            $.ajax({
                                type:'POST',
                                url:'{{ url("warehouse-stores/unit/deletebyselection") }}',
                                data:{
                                    unitIdArray: unit_id
                                },
