            ...
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        {!! Form::open(['route' => ['warehouse-store-adjustment.update', $lims_adjustment_data->id], 'method' => 'put', 'files' => true, 'id' => 'adjustment-form']) !!}
                        <div class="row">
                            <div class="col-md-12">
                                <div class="row">
                                    <div class="col-md-4">
                                        ...
                                        <div class="table-responsive mt-3">
                                            <table id="myTable" class="table table-hover order-list">
                                                ...
                                                <tbody>
                                                	...
                                                	<?php
                                                	   $basement = DB::table('basements')->find($raw_material_adjustment_data->product_id);
                                                	   $available_quantity = $basement ? $basement->qty : 0;
                                                	?>
                                                	@if($basement)
                                                	<tr>
                                                	<td>{{$basement->name}}</td>
                                                	<td>{{$basement->code}}</td>
                                                    <td>{{$raw_material_adjustment_data->unit_cost ?? $basement->cost}}<input type="hidden" name="unit_cost[]" value="{{$raw_material_adjustment_data->unit_cost ?? $basement->cost}}" /></td>
                                                	...
                                                	<input type="hidden" name="product_code[]" class="basement-code" value="{{$basement->code}}" />
                                                	<input type="hidden" class="basement-id" name="product_id[]" value="{{$basement->id}}" />
...
for(rowindex  =0; rowindex <= rownumber; rowindex++){
	...
	$('#lims_basementcodeSearch').on('input', function(){
	    var warehouse_id = $('#warehouse_id').val();
	    temp_data = $('#lims_basementcodeSearch').val();
	    if(!warehouse_id){
	        $('#lims_basementcodeSearch').val(temp_data.substring(0, temp_data.length - 1));
    ...
    $.get('{{ url("warehouse-store-adjustment/getbasement") }}/' + id, function(data) {
        ...
        $.each(basement_code, function(index) {
            if(exist_code.includes(basement_code[index])) {
                pos = exist_code.indexOf(basement_code[index]);
                basement_qty[index] = (parseFloat(basement_qty[index]) || 0) + (exist_qty[pos] || 0);
            }
            lims_basement_array.push(basement_code[index] + ' (' + basement_name[index] + ')'+ '|' + (unit_cost[index] || 0) + '|' + (basement_qty[index] || 0));
        });
    });
