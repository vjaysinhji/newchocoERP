...
<section>
    <div class="container-fluid">
        ...
        @can('rawmaterials-add')
            <button type="button" class="btn btn-info" data-toggle="modal" data-target="#raw-category-modal"><i class="dripicons-plus"></i> {{__("db.Add Category")}}</button>&nbsp;
        @endcan
    </div>
...
<!-- Create Modal -->
<div id="raw-category-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
  <div role="document" class="modal-dialog">
    <div class="modal-content">
        {!! Form::open(['route' => 'rawmaterials.category.store', 'method' => 'post', 'files' => true, 'id' => 'raw-category-form']) !!}
...
<script type="text/javascript">
    ...
    $("ul#rawmaterial #rawmaterial-category-menu").addClass("active");

    function confirmDelete() {
      if (confirm("If you delete category all raw materials under this category will also be deleted. Are you sure want to delete?")) {
          return true;
      }
      return false;
    }
    ...
    $(document).on("click", ".open-EditCategoryDialog", function(){
        ...
        var url = "{{ url('rawmaterials/category') }}/" + id + "/edit";
        $.get(url, function(data){
            ...
            // Set parent_id value and refresh selectpicker
            ...
            var updateUrl = "{{ url('rawmaterials/category') }}/" + (data.id || data['id']);
    ...
    $('#category-table').DataTable( {
        ...
        "serverSide": true,
        "ajax":{
            url:"{{ route('rawmaterials.category.data') }}",
            dataType: "json",
            type:"post"
        },
            ...
            },
            {
                ...
                action: function ( e, dt, node, config ) {
                    if(user_verified == '1') {
                        ...
                        if(category_id.length && confirm("If you delete category all raw materials under this category will also be deleted. Are you sure want to delete?")) {
                            $.ajax({
                                type:'POST',
                                url:'{{ url("rawmaterials/category/deletebyselection") }}',
                                data:{
