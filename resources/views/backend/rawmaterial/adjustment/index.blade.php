...
<section>
    <div class="container-fluid">
        <a href="{{route('rawmaterial-adjustment.create')}}" class="btn btn-info"><i class="dripicons-plus"></i> {{__('db.Add Adjustment')}}</a>
    </div>
                ...
                <tr data-id="{{$adjustment->id}}">
                    ...
                    <?php
                    	...
                    	foreach ($product_adjustment_data as $key => $product_adjustment) {
                            $raw_material = DB::table('raw_materials')->select('name','code')->find($product_adjustment->product_id);
                    	 	if($key)
                    	 		echo '<br>';
                    	 	echo $raw_material->name.'<br>'.$product_adjustment->qty.' x '.($product_adjustment->unit_cost ?? $raw_material->cost);
                    	 }
                    ?>
                    </td>
                    ...
                    <td>
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                                <li>
                                    <a href="{{ route('rawmaterial-adjustment.edit', $adjustment->id) }}" class="btn btn-link"><i class="dripicons-document-edit"></i> {{__('db.edit')}}</a>
                                </li>
                                <li class="divider"></li>
                                {{ Form::open(['route' => ['rawmaterial-adjustment.destroy', $adjustment->id], 'method' => 'DELETE'] ) }}
                                <li>
...
<script type="text/javascript">
    ...
    $("ul#rawmaterial #rawmaterial-adjustment-menu").addClass("active");
                ...
                action: function ( e, dt, node, config ) {
                    if(user_verified == '1') {
                        ...
                        if(adjustment_id.length && confirm("Are you sure want to delete?")) {
                            $.ajax({
                                type:'POST',
                                url:'{{ url("rawmaterial-adjustment/deletebyselection") }}',
                                data:{
