        ...
        <table id="stock-count-table" class="table stock-count-list">
            ...
            <tbody>
                ...
                <?php
                    ...
                    $category_name = [];
                    $brand_name = [];
                    $initial_file = 'stock_count/raw_material/' . $stock_count->initial_file;
                    $final_file = 'stock_count/raw_material/' . $stock_count->final_file;
                ?>
                <tr>
                    <td>{{$key}}</td>
                    ...
                    <td class="text-center">
                        <a download href="{{'stock_count/raw_material/'.$stock_count->initial_file}}" title="{{__('db.Download')}}"><i class="dripicons-copy"></i></a>
                    </td>
                    <td class="text-center">
                        @if($stock_count->final_file)
                        <a download href="{{'stock_count/raw_material/'.$stock_count->final_file}}" title="{{__('db.Download')}}"><i class="dripicons-copy"></i></a>
                        @endif
                    </td>
                    <td>
                        @if($stock_count->final_file)
...
<div id="createModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
    <div role="document" class="modal-dialog">
      <div class="modal-content">
        {!! Form::open(['route' => 'rawmaterial-stock-count.store', 'method' => 'post', 'files' => true]) !!}
...
<div id="finalizeModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
  <div role="document" class="modal-dialog">
    <div class="modal-content">
        {{ Form::open(['route' => 'rawmaterial-stock-count.finalize', 'method' => 'POST', 'files' => true] ) }}
      <div class="modal-header">
            ...
            <div class="container mt-3 pb-3">
                <div class="row border-bottom pb-2">
                    ...
                    <div class="col-md-12 text-center">
                        <i style="font-size: 15px;">{{__('db.Raw Material Stock Count')}}</i>
                    </div>
                </div>
                <br>
...
<script type="text/javascript">
    ...
    $("ul#rawmaterial #rawmaterial-stock-count-menu").addClass("active");
    ...
    $(document).on('click', '.final-report', function(){
        ...
        $.get('{{ url("rawmaterial-stock-count/stockdif") }}/' + stock_count[8], function(data){
            ...
            if(name_code){
                ...
                if( !parseInt(data[5]) ) {
                    htmlFooter = '<a class="btn btn-primary d-print-none" href="{{ url("rawmaterial-stock-count") }}/'+stock_count[8]+'/qty_adjustment"><i class="dripicons-plus"></i> {{__("db.Add Adjustment")}}</a>';
