...
<section>
    ...
    <div class="table-responsive">
        <table id="pickup-date-table" class="table" style="width: 100%">
            <thead>
            <tr>
            ...
            <tbody>
            @foreach($pickupDates as $index => $row)
                <tr data-id="{{ $row->id }}">
                    <td></td>
                    <td>{{ $index + 1 }}</td>
                    <td>{{ $row->date }}</td>
                            ...
                            <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                                <li>
                                    <button type="button"
                                            ...
                                            data-id="{{ $row->id }}"
                                            data-date="{{ $row->date }}"
                                            data-reason_en="{{ $row->reason_en }}"
                                            data-reason_ar="{{ $row->reason_ar }}"
                                ...
                                {{ Form::open(['route' => ['disabled-pickup-dates.destroy', $row->id], 'method' => 'DELETE']) }}
                                <li>
    ...
    <div role="document" class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 id="createModalLabel" class="modal-title">New Pickup Date</h5>
                <button type="button" data-dismiss="modal" aria-label="Close" class="close"><span aria-hidden="true"><i class="dripicons-cross"></i></span></button>
            </div>
            <div class="modal-body">
                {!! Form::open(['route' => 'disabled-pickup-dates.store', 'method' => 'post']) !!}
                <div class="row">
                    <div class="col-md-12 form-group">
                        <label>Pickup Date</label>
                        <input type="date" name="date" class="form-control input-tip datetime" placeholder="dd / mm / yyyy" required>
                    </div>
                    <div class="col-md-12 form-group">
                        <label>Why Disabled(En)</label>
                        <input type="text" name="reason_en" class="form-control">
                    </div>
                    <div class="col-md-12 form-group">
                        <label>Why Disabled(Ar)</label>
...
<div id="edit-modal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true" class="modal fade text-left">
    <div role="document" class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 id="editModalLabel" class="modal-title">Edit Pickup Date</h5>
                <button type="button" data-dismiss="modal" aria-label="Close" class="close"><span aria-hidden="true"><i class="dripicons-cross"></i></span></button>
            </div>
            <div class="modal-body">
                {!! Form::open(['route' => ['disabled-pickup-dates.update', 0], 'method' => 'put', 'id' => 'edit-form']) !!}
                <div class="row">
                    <div class="col-md-12 form-group">
                        <label>Pickup Date</label>
                        <input type="date" name="date" class="form-control input-tip datetime" required>
                    </div>
                    <div class="col-md-12 form-group">
                        <label>Why Disabled(En)</label>
                        <input type="text" name="reason_en" class="form-control">
...
<script type="text/javascript">
    ...
    $("ul#delivery-operation #disabled-pickup-date-menu").addClass("active");
    ...
    $(document).on('click', '.edit-btn', function () {
        var id = $(this).data('id');
        var action = '{{ route('disabled-pickup-dates.update', 0) }}';
        action = action.replace('/0', '/' + id);
        $('#edit-form').attr('action', action);

        $("#edit-form input[name='date']").val($(this).data('date'));
        $("#edit-form input[name='reason_en']").val($(this).data('reason_en'));
        $("#edit-form input[name='reason_ar']").val($(this).data('reason_ar'));
    ...
    $(document).on('change', '.status-toggle', function(){
        ...
        var url = '{{ route("disabled-pickup-dates.inlineStatus", ":id") }}'.replace(':id', id);
    ...
    $(document).on('change', '.sort-order-input', function(){
        ...
        var url = '{{ route("disabled-pickup-dates.inlineSort", ":id") }}'.replace(':id', id);
